...
def swirling_uni_pdf(theta, arm_index, max_distance, a=0.6, b=0.1, k=1, M=6):
    """
...
def generate_uni_star_points(num_points=10000, sigma=0.5, arm_size=5):
    """
    ...
    for arm_index in range(arm_size):  # Assuming 5 arms
        ...
        for _ in range(points_for_arm):  # Generate the specified number of points for each arm
            ...
            r = swirling_uni_pdf(theta, arm_index, max_distance)
...
def generate_uni_star_fish(num_points = 10000):
    x = generate_uni_star_points(num_points)
