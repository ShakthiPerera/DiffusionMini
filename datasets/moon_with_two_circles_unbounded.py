...
class MoonWithTwoCirclesUnboundedDataset(BaseDataset2D):
    ...
    def generate_crescent_with_circles(self):
        ...
        angles1 = 2 * np.pi * self.pdf(distribution="uniform", size=self.circle_density)  # Random angles for circular distribution
        angles2 = 2 * np.pi * self.pdf(distribution="uniform", size=self.circle_density)
        radii1 = self.circle_radius * np.sqrt(self.pdf(distribution="uniform", size=self.circle_density))  # Scaled radii for dense clustering
        radii2 = self.circle_radius * np.sqrt(self.pdf(distribution="uniform", size=self.circle_density))
        
        radii1 += 0.05 * self.pdf(distribution="standard_normal", size=self.circle_density)
        radii2 += 0.05 * self.pdf(distribution="standard_normal", size=self.circle_density)
